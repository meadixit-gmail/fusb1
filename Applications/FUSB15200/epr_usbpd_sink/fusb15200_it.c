...
void ADC_IRQHandler()
{
    HAL_ADC_CHx_STAT_T stat_ch0, stat_ch6, stat_ch3, stat_ch9;
    gSleepInt = true;
    stat_ch0  = ADC_DRIVER.ChannelStatus(ADC_CH0);
    stat_ch6  = ADC_DRIVER.ChannelStatus(ADC_CH6);
    stat_ch3  = ADC_DRIVER.ChannelStatus(ADC_CH3);
    stat_ch9  = ADC_DRIVER.ChannelStatus(ADC_CH9);
    ...
    if (stat_ch3.alarml)
    {
        ADC_DRIVER.ChannelSetLowAlarmInterrupt(ADC_CH3, false);
    }
    if (stat_ch3.alarmh)
    {
        ADC_DRIVER.ChannelSetHighAlarmInterrupt(ADC_CH3, false);
    }

    /* NTC_B */
    if (stat_ch9.alarml)
    {
        ADC_DRIVER.ChannelSetLowAlarmInterrupt(ADC_CH9, false);
    }
    if (stat_ch9.alarmh)
    {
        ADC_DRIVER.ChannelSetHighAlarmInterrupt(ADC_CH9, false);
    }
